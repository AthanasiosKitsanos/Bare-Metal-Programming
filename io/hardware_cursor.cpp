...
{
    [[gnu::regparm(2)]]
    void hardware_cursor::write_register(uint8_t index, uint8_t value) noexcept
    {
    ...
    [[gnu::regparm(1)]]
    uint8_t hardware_cursor::read_register(uint8_t index) noexcept
    {
        outb(command_port, index);
        return inb(data_port);
    }

    void hardware_cursor::enable(uint8_t start, uint8_t end) noexcept
    {
    ...
    [[gnu::regparm(1)]]
    void hardware_cursor::set_position(const uint16_t position) noexcept
    {
    ...
    [[gnu::regparm(1)]]
    void hardware_cursor::set_display_start(const uint16_t position) noexcept
    {

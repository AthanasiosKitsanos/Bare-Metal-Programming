...
namespace kernel
{
    ...
    {
        uint8_t master_mask{io::inb(master_data)};
        uint8_t slave_mask{io::inb(slave_data)};

        io::outb(master_command, enable);
        io_wait();
        io::outb(slave_command, enable);
        io_wait();

        io::outb(master_data, offset_1);
        io_wait();
        io::outb(slave_data, offset_2);
        io_wait();

        io::outb(master_data, master_bit);
        io_wait();
        io::outb(slave_data, slave_bit);
        io_wait();

        io::outb(master_data, x86_mode);
        io_wait();
        io::outb(slave_data, x86_mode);
        io_wait();
    ...
    {
        if(vector > 7) io::outb(slave_command, eoi_command);
        io::outb(master_command, eoi_command);
    }

    void mask_all_except_timer_and_keyboard() noexcept
    {
        io::outb(master_data, 0xFC);
        io_wait();

...
namespace kernel
{
    void initialize_pit(uint32_t frequency) noexcept
    {
        ...
        io::outb(command_port, channel_0_lobyte_hibyte_mode_3_binary);
        io::outb(channel_0_data_port, static_cast<uint8_t>(divisor & 0x00FF));
        io::outb(channel_0_data_port, static_cast<uint8_t>((divisor >> 8) & 0x00FF));
    }
}
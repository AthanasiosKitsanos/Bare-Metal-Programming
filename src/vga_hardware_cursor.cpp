...
{
    uint8_t cursor_start{static_cast<uint8_t>(read_register(0x0A) & 0xC0)};
    cursor_start |= static_cast<uint8_t>(start & 0x1F);
    write_register(0x0A, cursor_start);

    uint8_t cursor_end{static_cast<uint8_t>(read_register(0x08) & 0xE0)};
    cursor_end |= static_cast<uint8_t>(end & 0x1F);
...
void vga_hardware_cursor::set_position(size_t position) noexcept
{
    ...
    write_register(0x0F, static_cast<uint8_t>((pos >> 8) & 0x00FF));
}